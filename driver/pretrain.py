...
from model.language_model import BertForSA
...
class BERTTrainer:
    ...
    def __init__(self, bert: BERT, vocab_size: int,model:BERTLM,
            ...
            for epoch in range(hp.epochs):
                ...
                for i, data in data_iter:
                    ...
                    if self.step % hp.save_runs == 0 and data["mlm_input"].size(0) == hp.batch_size: # 不足batch数量则不采样
                        ...
                        for module in self.model.modules():  # param.clone().cpu().data.numpy()   .module
                            for name, param in module.named_parameters():
                                self.train_model_param_writer.add_histogram(f"Epoch{epoch}_train_{name}",
                                                                            param.clone().cpu().data.numpy(), self.step)
            ...
            for i, data in data_iter:
                ...
                for j,q in enumerate(mask_lm_output):
                    ...
                    for i,z in enumerate(input):
                        if z.item() == 4:
                            char +='['+vocab.index2char(q[i].argmax().item())+']'
                        else:
                    ...
                    for i, z in enumerate(label):
                        ...
                        if z.item() !=0:
                            ...
                            info = vocab.index2char(z.item())+' ' +(str)(softmax[z.item()])+ '  '
                            for zz in topk:
                                info += ' '+vocab.index2char(zz.item())+' '+(str)(softmax[zz.item()])
                            print(info)
                    print(char)
